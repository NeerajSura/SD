...
from models_sd.unet import Unet
...
def train(args):
    ...
    # Load VAE only if latents are not to be used or not present
    if not im_dataset.use_latents:
        print('Loading VQVAE model as latents are not present')
        vae = VQVAE(im_channels=autoencoder_model_config['im_channels'],
                    model_config=autoencoder_model_config).to(device)
        vae.eval()

        # Load vae chkpts if found
        if os.path.exists(os.path.join(train_config['task_name'], train_config['vqvae_autoencoder_ckpt_name'])):
            print('loading vae checkpoints')
    ...
    optimizer = Adam(params=model.parameters(), lr=train_config['ldm_lr'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', dest='config_path',
                        default='config_sd/mnist.yaml', type=str)
