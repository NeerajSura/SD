...
from models_sd.lpips import LPIPS
from models_sd.discriminator import Discriminator
...
def train(args):
    ...
    disc_crtierion = torch.nn.MSELoss()

    lpips_model = LPIPS().eval().to(device)
    discriminator = Discriminator(im_channels=dataset_config['im_channels']).to(device)

    optimizer_d = Adam(params=discriminator.parameters(), lr=train_config['autoencoder_lr'], betas=(0.5, 0.999))
    optimizer_g = Adam(params=model.parameters(), lr=train_config['autoencoder_lr'], betas=(0.5, 0.999))
    ...
    for epoch_idx in range(num_epochs):
        ...
        for im in tqdm(data_loader):
            ...
            # g_loss = recon_loss + codebook + commitment loss
            g_loss = (recon_loss +
            ...
            if step_count > disc_step_start:
                ...
                disc_fake_loss = disc_crtierion(disc_fake_pred,
            ...
            if step_count > disc_step_start:
                ...
                disc_fake_loss = disc_crtierion(disc_fake_pred,
                                                torch.zeros(disc_fake_pred.shape,
                                                            device=disc_fake_pred.device))
                disc_real_loss = disc_crtierion(disc_real_pred,
